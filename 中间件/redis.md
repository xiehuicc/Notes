...
> 布隆过滤器是一个很长的二进制数组（初始值都为0的bit数组） + 多个随机hash算法映射函数，主要用于判断一个元素是否在集合中。（bitmap + 分布均与的多个hash算法构成的集合）
>
> 本质就是判断具体数据存不存在一个大的集合中
>
> 布隆过滤器就是一种类似set的数据结构，只是统计不太准确。（可能hash冲突，相同的bit位被多次映射且置为1）
...
- 高效的插入和查询，占用空间少
- 一个元素判断结果为存在时不一定存在，但判断不存在时，一定不存在。
- 布隆过滤器可以添加元素，但不能删除元素，因为删除元素会导致误判率增加（存在hash冲突，如果多个数据冲突，而这些在bitmap中同一位置，删除将导致该位置bit从1变为0（0表示不存在））
- 误判只会发生在过滤器中没有添加过的元素，对于添加过的元素不会发生误判。
...
>  redis缓存中没有该条数据，当数据库中也没有该数据时，每次查询都要访问数据库，这就是缓存穿透。
>
> 可能会给数据库带来压力，导致数据库崩溃

方法：

把已存在数据的key存在布隆过滤器中，相当于redis前挡了一个布隆过滤器。

但有新的请求时，先查布隆过滤器，无则直接返回，有则查询redis，redis无则查询数据库

- 黑名单/白名单校验

  > 发现存在黑名单中，就执行特定操作。比如：垃圾邮件 ..
